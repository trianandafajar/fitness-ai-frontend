...
export default function StepShell({
  ...
  return (
    <div className="flex flex-1 flex-col pb-28">
      ...
      <div className="fixed inset-x-0 bottom-0 z-30 rounded-t-2xl border-t border-line bg-card">
        <div className="mx-auto flex w-full max-w-100 items-center gap-3 px-5 py-2.5">
          {onBack && (
            <button
              type="button"
              onClick={onBack}
              className="flex h-10 flex-1 items-center justify-center rounded-[10px] border-[1.5px] border-line bg-card px-3.25 text-[14px] font-semibold text-ink transition-colors hover:border-ink-faint"
            >
              Back
            </button>
          )}
