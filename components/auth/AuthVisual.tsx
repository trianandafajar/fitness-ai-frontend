...
export default function AuthVisual({ tag, heading, sub, stats }: AuthVisualProps) {
    return (
        <div className="relative hidden flex-col items-center justify-center overflow-hidden bg-gradient-to-br from-orange to-orange-deep p-12 md:flex">
            <svg
            ...
            <div className="relative z-[2] w-full max-w-[380px] text-left">
                <div className="mb-5 inline-block rounded-full bg-white/[0.16] px-3 py-[5px] text-xs font-semibold tracking-wide text-white">
                    {tag}
                </div>
                <div className="mb-[14px] font-display text-[26px] font-bold leading-[1.3] tracking-tight text-white">
                    {heading}
                </div>
                ...
                {stats && stats.length > 0 && (
                    <div className="relative z-[2] flex flex-col gap-3">
                        {stats.map((stat) => (
                            <div
                                key={stat.label}
                                className="flex items-center justify-between rounded-xl border border-white/25 bg-white/[0.14] px-4 py-[14px] backdrop-blur-md"
                            >
