...
export default function TodayPlanCard({ schedule, attendanceToday, loading }: TodayPlanCardProps) {
  if (loading) {
    return (
  ...
  const checkedIn = attendanceToday?.status === "verified" || attendanceToday?.status === "pending";
    ...
    <div className="rounded-2xl border border-line bg-white p-5.5">
        ...
        {schedule.exercises.map((ex, i) => (
          <div
            ...
            className={`flex items-center justify-between py-2.75 ${
              i !== schedule.exercises.length - 1 ? "border-b border-line" : ""
            }`}
          >
      ...
      <div className="mt-4 flex gap-2.5">
        {checkedIn ? (
          ...
          <ButtonPrimary type="button" className="flex-1 py-2.75 text-[13.5px]">
            Start Workout
          </ButtonPrimary>
        )}
